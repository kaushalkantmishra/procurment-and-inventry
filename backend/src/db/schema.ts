import { pgTable, serial, text, integer, decimal, boolean, date, timestamp, varchar, check } from 'drizzle-orm/pg-core';
...
export const tblCategories = pgTable('tbl_categories', {
    ...
    parent_category_id: integer('parent_category_id').references((): any => tblCategories.id),
...
export const categoriesRelations = relations(tblCategories, ({ one, many }) => ({
...
export const tblItems = pgTable('tbl_items', {
    ...
    unit_of_measure: varchar('unit_of_measure', { length: 20 }).references(() => tblUnits.unit_id),
    unit_cost: decimal('unit_cost', { precision: 15, scale: 2 }).default('0'),
    selling_price: decimal('selling_price', { precision: 15, scale: 2 }).default('0'),
...
export const tblPurchaseRequests = pgTable('tbl_purchase_requests', {
    ...
    estimated_unit_price: decimal('estimated_unit_price', { precision: 15, scale: 2 }),
    total_estimated_cost: decimal('total_estimated_cost', { precision: 15, scale: 2 }),
...
export const purchaseRequestsRelations = relations(tblPurchaseRequests, ({ one }) => ({
    item: one(tblItems, {
...
export const tblPoLines = pgTable('tbl_po_lines', {
    id: serial('id').primaryKey(),
    po_id: integer('po_id').references(() => tblPurchaseOrders.id).notNull(),
...
export const purchaseOrdersRelations = relations(tblPurchaseOrders, ({ many }) => ({
...
export const poDistributionsRelations = relations(tblPoDistributions, ({ one }) => ({
...
export const tblGrnDetails = pgTable('tbl_grn_details', {
    id: serial('id').primaryKey(),
    grn_id: integer('grn_id').references(() => tblGrnHeaders.id).notNull(),
...
export const grnHeadersRelations = relations(tblGrnHeaders, ({ one, many }) => ({
    po: one(tblPurchaseOrders, {
...
export const tblReceiptHeaders = pgTable('tbl_receipt_headers', {
    ...
    store_id: varchar('store_id', { length: 20 }).references(() => tblWarehouses.warehouse_code),
    cashier_id: varchar('cashier_id', { length: 50 }),
    customer_id: varchar('customer_id', { length: 50 }),
    subtotal: decimal('subtotal', { precision: 15, scale: 2 }).default('0'),
    discount_total: decimal('discount_total', { precision: 15, scale: 2 }).default('0'),
...
export const tblReceiptLines = pgTable('tbl_receipt_lines', {
    id: serial('id').primaryKey(),
    receipt_id: integer('receipt_id').references(() => tblReceiptHeaders.id).notNull(),
    item_id: integer('item_id').references(() => tblItems.id).notNull(),
    product_snapshot_name: varchar('product_snapshot_name', { length: 100 }),
    quantity: integer('quantity').notNull(),
    unit_price: decimal('unit_price', { precision: 15, scale: 2 }).notNull(),
    line_discount: decimal('line_discount', { precision: 15, scale: 2 }).default('0'),
    line_tax_rate: decimal('line_tax_rate', { precision: 5, scale: 2 }).default('0'),
    line_total: decimal('line_total', { precision: 15, scale: 2 }).notNull(),
    status: integer('status').default(1),
    created_at: timestamp('created_at').defaultNow().notNull(),
    updated_at: timestamp('updated_at').defaultNow(),
...
export const tblPayments = pgTable('tbl_payments', {
    ...
    payment_amount: decimal('payment_amount', { precision: 15, scale: 2 }).notNull(),
...
export const receiptHeadersRelations = relations(tblReceiptHeaders, ({ one, many }) => ({
...
export const inventoryTransactionsRelations = relations(tblInventoryTransactions, ({ one }) => ({
    item: one(tblItems, {
